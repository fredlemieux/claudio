import { useState } from "react";
import type { AlgorithmPhase, ISCriterion } from "./AlgorithmTracker";

interface ISCPanelProps {
  phases: AlgorithmPhase[];
  criteria: ISCriterion[];
}

const STATUS_ICONS = {
  pending: <span className="w-3 h-3 rounded border border-border inline-block shrink-0" />,
  in_progress: <span className="w-3 h-3 rounded border border-blue-500 bg-blue-500/20 inline-block animate-pulse shrink-0" />,
  completed: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-3 h-3 text-green-400 shrink-0">
      <path fillRule="evenodd" d="M12.416 3.376a.75.75 0 0 1 .208 1.04l-5 7.5a.75.75 0 0 1-1.154.114l-3-3a.75.75 0 0 1 1.06-1.06l2.353 2.353 4.493-6.74a.75.75 0 0 1 1.04-.207Z" clipRule="evenodd" />
    </svg>
  ),
  failed: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-3 h-3 text-red-400 shrink-0">
      <path d="M5.28 4.22a.75.75 0 0 0-1.06 1.06L6.94 8l-2.72 2.72a.75.75 0 1 0 1.06 1.06L8 9.06l2.72 2.72a.75.75 0 1 0 1.06-1.06L9.06 8l2.72-2.72a.75.75 0 0 0-1.06-1.06L8 6.94 5.28 4.22Z" />
    </svg>
  ),
};

function PhaseStrip({ phases }: { phases: AlgorithmPhase[] }) {
  return (
    <div className="flex gap-0.5">
      {phases.map((phase) => {
        const colors = {
          pending: "bg-surface-2 text-text-tertiary",
          active: "bg-blue-500/20 text-blue-400",
          completed: "bg-green-500/15 text-green-400",
        };
        return (
          <div
            key={phase.id}
            className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] ${colors[phase.status]}`}
            title={phase.name}
          >
            <span className="text-[10px]">{phase.icon}</span>
            {phase.status === "active" && (
              <span className="w-1 h-1 rounded-full bg-blue-400 animate-pulse" />
            )}
          </div>
        );
      })}
    </div>
  );
}

export function ISCPanel({ phases, criteria }: ISCPanelProps) {
  const [expanded, setExpanded] = useState(true);
  const completedCount = criteria.filter((c) => c.status === "completed").length;
  const failedCount = criteria.filter((c) => c.status === "failed").length;
  const hasPhases = phases.some((p) => p.status !== "pending");
  const hasCriteria = criteria.length > 0;

  // Don't render anything if there's no algorithm activity
  if (!hasPhases && !hasCriteria) return null;

  return (
    <div className="border-t border-border">
      {/* Header — always visible, acts as toggle */}
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center justify-between px-3 py-2 hover:bg-surface-2 transition-colors"
      >
        <div className="flex items-center gap-2">
          <span className="text-[10px]">♻️</span>
          <span className="text-[10px] font-semibold text-text-primary">ISC</span>
          {hasCriteria && (
            <span className="text-[10px] text-text-secondary">
              {completedCount}/{criteria.length}
              {failedCount > 0 && <span className="text-red-400 ml-1">{failedCount} failed</span>}
            </span>
          )}
        </div>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 16 16"
          fill="currentColor"
          className={`w-3 h-3 text-text-tertiary transition-transform ${expanded ? "rotate-180" : ""}`}
        >
          <path fillRule="evenodd" d="M4.22 6.22a.75.75 0 0 1 1.06 0L8 8.94l2.72-2.72a.75.75 0 1 1 1.06 1.06l-3.25 3.25a.75.75 0 0 1-1.06 0L4.22 7.28a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" />
        </svg>
      </button>

      {/* Progress bar — always visible when criteria exist */}
      {hasCriteria && (
        <div className="px-3 pb-1">
          <div className="h-1 bg-border rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-blue-500 to-green-500 rounded-full transition-all duration-500"
              style={{ width: `${(completedCount / criteria.length) * 100}%` }}
            />
          </div>
        </div>
      )}

      {expanded && (
        <div className="px-3 pb-3">
          {/* Phase strip */}
          {hasPhases && (
            <div className="mb-2">
              <PhaseStrip phases={phases} />
            </div>
          )}

          {/* Criteria list */}
          {hasCriteria && (
            <div className="space-y-0.5 max-h-[300px] overflow-y-auto">
              {criteria.map((c) => (
                <div key={c.id} className="flex items-start gap-1.5 py-0.5">
                  <div className="mt-0.5">{STATUS_ICONS[c.status]}</div>
                  <span className={`text-[10px] leading-relaxed ${
                    c.status === "completed" ? "text-text-secondary line-through" :
                    c.status === "failed" ? "text-red-300" :
                    c.status === "in_progress" ? "text-blue-300" :
                    "text-text-interactive"
                  }`}>
                    {c.description}
                  </span>
                </div>
              ))}
            </div>
          )}
        </div>
      )}
    </div>
  );
}
